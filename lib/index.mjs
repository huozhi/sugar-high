...
function isStringQuotation(chr) {
  return chr === '"' || chr === "'" || isStrTemplateChr(chr)
}
...
export function tokenize(code) {
  ...
  for (let i = 0; i < code.length; i++) {
    ...
    if (isStringQuotation(curr)) {
      ...
      if (__strQuote) {
        if (curr === '`') __strTemplateQuoteStack--
        __strQuote = __strTemplateQuoteStack > 0 ? '`' : false
      } else if (!inStrTemplateLiterals()) {
        if (curr === '`') __strTemplateQuoteStack++
        __strQuote = curr
      }
      current = curr
    ...
    if (inStrTemplateLiterals()) {
      // is single line quoted string
      if ((__strQuote === `'` || __strQuote === `"`) && prev !== `\\`) {
        append()
        __strQuote = curr === __strQuote ? false : curr
        current = curr
        append(T_STRING)
        continue
      } else if (__strQuote === '`') {
    ...
    // string quotation
    if (isQuotationChar || isStringTemplateLiterals || (__strQuote === '"' || __strQuote === "'")) {
      current += curr
    } else if (isRegexChar) {
      append()
