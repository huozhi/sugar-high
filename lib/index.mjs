...
export function tokenize(code) {
  ...
  /** @type {{ tag: boolean; child: boolean }} */
  const jsx = { tag: false, child: false }
  ...
  for (let i = 0; i < code.length; i++) {
    ...
    if (!jsx.tag && !string.entered && curr === '<') {
      jsx.tag = true
      jsx.child = false
    }
    ...
    if (
      ...
      if (isIdentifierChar(curr) !== isIdentifierChar(current[current.length - 1])) {
        append()
        current = curr
      } else {
        current += curr
