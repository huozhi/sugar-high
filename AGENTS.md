...
- For React layout changes, install Chromium with
  `pnpm --filter @sugar-high/react exec playwright install chromium` and run
  `pnpm --filter @sugar-high/react test:browser`. These checks measure rendered layout without
  starting or building the documentation site.
